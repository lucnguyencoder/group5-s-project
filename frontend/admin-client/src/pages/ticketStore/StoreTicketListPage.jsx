...
const StoreTicketListPage = () => {
  ...
  const filterTICTickets = (ticketsArray) => {
    return ticketsArray.filter(ticket => {
      ...
      return String(ticketCode).startsWith('STC');
    });
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background">
        <SetTitle title="Store Tickets" />
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-2xl font-bold text-foreground mb-6">Store Support Tickets (STC)</h1>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-2xl font-bold text-foreground mb-6">STC Support Tickets (TIC)</h1>
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-foreground">Store Support Tickets (STC)</h1>
          <div className="text-sm text-muted-foreground">
            STC tickets only • Total: {tickets.length}
          </div>
        </div>
        ...
        {tickets.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground text-lg mb-2">No STC tickets found</p>
            <p className="text-muted-foreground text-sm">Store tickets (STC) will appear here when they are created</p>
          </div>
        ) : (
          <div className="space-y-4">
            {tickets.map((ticket) => (
              <Card
                ...
                onClick={() => navigate(`/tickets/store/${ticket.ticket_id}`)}
              >
...
export default StoreTicketListPage;